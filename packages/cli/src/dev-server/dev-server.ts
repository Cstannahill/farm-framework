...
export class FarmDevServer extends EventEmitter<DevServerEvents> {
  ...
  private async loadConfiguration(): Promise<void> {
    ...
    try {
      ...
      // Use the proper config loader from CLI package
      const { ConfigLoader } = await import("../core/config");
      const configLoader = new ConfigLoader();
      const config = await configLoader.loadConfig({
        configPath,
        cwd: this.state.projectPath!,
        validate: true
      });
  ...
  private getServiceConfigurations() {
    ...
    if (this.options.frontendOnly) {
      const allConfigs = this.serviceConfigManager.getServiceConfigs(config, projectPath);
      const filteredConfigs = allConfigs.filter((c: ServiceConfig) => c.key === "frontend");
      console.log('🔍 --frontend-only filtering:', {
        allConfigs: allConfigs.map(c => ({ name: c.name, key: c.key })),
        filteredConfigs: filteredConfigs.map(c => ({ name: c.name, key: c.key }))
      });
  ...
  private async startServicesInPhases(serviceConfigs: any[]): Promise<void> {
    ...
    for (const phase of phases) {
      ...
      const phaseServices = serviceConfigs.filter((config) =>
        phase.services.some(service => service.key === config.key)
      );
  ...
  private groupServicesByPhases(
    ...
    console.log('🔍 groupServicesByPhases called with services:', serviceConfigs.map(s => ({ name: s.name, key: s.key })));
    ...
    serviceConfigs.forEach((service) => {
      ...
      // Map service keys to appropriate phases
      switch (service.key) {
        case "database":
          phaseName = "database";
          break;
        ...
        case "frontend":
          ...
          break;
        default:
          // Default to backend phase for unknown services
          phaseName = "backend";
      }

      console.log(`🔍 Service ${service.name} (${service.key}) -> phase: ${phaseName}`);

      const phase = phases.find((p) => p.name === phaseName);
      if (phase) {
        phase.services.push(service);
        console.log(`✅ Added ${service.name} to ${phaseName} phase`);
      } else {
        console.log(`❌ Phase ${phaseName} not found for service ${service.name}`);
      }
    });

    console.log('🔍 Final phases:', phases.map(p => ({ name: p.name, serviceCount: p.services.length, services: p.services.map(s => s.name) })));
    return phases;
  }
  ...
  private getServicePort(serviceName: string, ports: any): number | undefined {
    switch (serviceName) {
      case "frontend":
        return ports.frontend || 3000;
      case "backend":
        return ports.backend || 8000;
      case "ollama":
        return ports.ollama || 11434;
      default:
        return undefined;
    }
  }
  ...
  private printStartupSummary(): void {
    ...
    // Show main dev server proxy port
    this.logger.keyValue("🌾 FARM Dev Server", `http://localhost:${this.options.port} (proxy)`);

    const services = this.processManager.getAllServices();
    services.forEach((service) => {
      const statusIcon = service.status === "healthy" ? "✅" : "⚠️";
      const servicePort = this.getServicePort(service.name, ports);
      const portInfo = servicePort ? ` (port ${servicePort})` : "";
      this.logger.keyValue(`${statusIcon} ${service.name}${portInfo}`, service.status);
    });
