...
export class ConfigLoader {
  ...
  async loadConfig(
    ...
    try {
      ...
      if (resolvedPath.endsWith('.ts')) {
        // Handle TypeScript files by converting to JavaScript
        configModule = await this.loadTypeScriptConfig(resolvedPath);
      } else {
  ...
  private registerTypeScriptSupport(): void {
    try {
      // For now, we'll handle TypeScript files by converting them to JavaScript
      // In a full implementation, you'd use tsx or ts-node here
      logger.debug("TypeScript support registered (basic mode)");
    } catch (error) {
      logger.warn(
  ...
  private async loadTypeScriptConfig(configPath: string): Promise<any> {
    // Simple TypeScript config loader that strips TypeScript syntax
    // This is a basic implementation - in production you'd use tsx or ts-node
