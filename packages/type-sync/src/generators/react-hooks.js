...
export class ReactHookGenerator {
    ...
    extractHooksFromSchema(schema, opts) {
        const hooks = [];
        if (schema.paths) {
            for (const [path, pathItem] of Object.entries(schema.paths)) {
                ...
                for (const [method, operation] of Object.entries(pathItem)) {
                    ...
                    const hook = this.createHookFromOperation(method, path, operation, opts);
                    if (hook) {
                        hooks.push(hook);
                    }
                }
    ...
    createHookFromOperation(method, path, operation, opts) {
        ...
        const isInfiniteQuery = isQuery &&
            opts.enableInfiniteQueries &&
            this.isInfiniteQueryCandidate(operation, path);
        return {
            name: this.generateHookName(operationId, isInfiniteQuery ? "infiniteQuery" : isQuery ? "query" : "mutation"),
    ...
    generateHooksContent(hooks, opts) {
    ...
    generateHookImplementation(hook, opts) {
        switch (hook.type) {
            case "query":
    ...
    generateInfiniteQueryHook(hook, opts) {
        const params = this.generateHookParameters(hook);
        const queryKey = this.generateQueryKey(hook);
        let hookContent = `export function ${hook.name}(${params}) {\n`;
    ...
    generateMutationHook(hook, opts) {
    ...
    generateHookParameters(hook) {
        ...
        if (hook.queryParams && hook.queryParams.length > 0) {
            params.push(`params?: { ${hook.queryParams.map((p) => `${p}?: any`).join("; ")} }`);
        }
        // Options parameter
        params.push(`options?: Omit<Parameters<typeof useQuery>[0], 'queryKey' | 'queryFn'>`);
        return params.join(", ");
    }
    generateMutationParameters(hook) {
        const params = [];
        // Options parameter for mutations
        params.push(`options?: Omit<Parameters<typeof useMutation>[0], 'mutationFn'>`);
        return params.join(", ");
    }
    generateQueryKey(hook) {
    ...
    generateOptimisticUpdates(hook) {
    ...
    generateInvalidationLogic(hook) {
    ...
    getDomainFromOperationId(operationId) {
    ...
    generateHookName(operationId, type) {
    ...
    getResponseType(operation) {
        ...
        if (successResponse?.content?.["application/json"]?.schema) {
            return this.getTypeFromSchema(successResponse.content["application/json"].schema);
        }
        return "unknown";
    }
    ...
    isInfiniteQueryCandidate(operation, path) {
        ...
        return listKeywords.some((keyword) => path.toLowerCase().includes(keyword) ||
