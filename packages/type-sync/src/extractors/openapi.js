...
export class OpenAPIExtractor {
    ...
    async extractFromFastAPI(apiPath, outputPath, options = {}) {
        ...
        try {
            ...
            throw new Error(`Failed to extract OpenAPI schema: ${error instanceof Error ? error.message : "Unknown error"}`);
        }
    }
    /**
     * Try to fetch schema from already running server
     */
    async tryRunningServer(options) {
        try {
     ...
     */
    async tryCachedSchema(outputPath, options) {
        if (!options.enableCache)
            return null;
        // Check memory cache first
        if (this.lastSchemaCache &&
        ...
        for (let attempt = 0; attempt <= options.retries; attempt++) {
            try {
                ...
                if (attempt < options.retries) {
                    console.warn(`Schema fetch attempt ${attempt + 1} failed, retrying in ${options.retryDelay}ms...`);
