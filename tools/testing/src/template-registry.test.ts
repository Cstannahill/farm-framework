...
describe('Template Registry', () => {
    ...
    async function createTestTemplates() {
        ...
        for (const file of baseFiles) {
            await fs.writeFile(path.join(baseDir, file), `# Mock content for ${file}`);
        }
        ...
        for (const file of baseApiFiles) {
            await fs.writeFile(path.join(baseApiDir, file), `# Mock content for ${file}`);
        }
        ...
        for (const file of baseWebFiles) {
            await fs.writeFile(path.join(baseWebDir, file), `# Mock content for ${file}`);
        }

        // Create basic template files
        const basicDir = path.join(templatesDir, 'basic');
        await fs.ensureDir(basicDir);
        await fs.writeFile(path.join(basicDir, 'package.json.hbs'), '{"name": "{{projectName}}"}');
        await fs.writeFile(path.join(basicDir, 'farm.config.ts.hbs'), 'export default {}');
        await fs.writeFile(path.join(basicDir, 'README.md.hbs'), '# {{projectName}}');
    }
    ...
    describe('Template File Validation', () => {
        ...
        it('should validate files with features', async () => {
            const validation = await registry.validateTemplateFiles('basic', ['web', 'api']);
            expect(validation).toBeDefined();
    ...
    describe('Template Inheritance', () => {
        it('should resolve inheritance chain correctly', () => {
            ...
            const templateFiles = files.filter(file => file.inheritFromBase !== true);
        ...
        it('should handle templates without inheritance', () => {
            ...
            const inheritedFiles = files.filter(file => file.inheritFromBase === true);
    ...
    describe('Edge Cases', () => {
        ...
        it('should handle features array with duplicates', () => {
            const files = registry.resolveFiles('basic', ['web', 'web', 'api', 'api']);
            expect(files).toBeDefined();
            expect(Array.isArray(files)).toBe(true);
        });
