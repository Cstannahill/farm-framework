...
describe('Template Processing System', () => {
    ...
    describe('TemplateProcessor', () => {
        ...
        it('should handle template compilation errors gracefully', async () => {
            ...
            await fs.writeFile(templatePath, 'Hello {{invalid syntax}}!');
    ...
    describe('Handlebars Helpers', () => {
        ...
        describe('AI Provider Helpers', () => {
            it('should check AI provider with if_ai_provider', () => {
                const template = '{{#if_ai_provider ollama}}Ollama enabled{{else}}Ollama disabled{{/if_ai_provider}}';
                const compiled = handlebars.compile(template);
            ...
            it('should check specific AI providers', () => {
                const templates = {
                    ...
                    '{{#has_ai_enabled}}AI enabled{{else}}AI disabled{{/has_ai_enabled}}': 'AI enabled'
                };
        ...
        describe('Template Helpers', () => {
            it('should check template type with if_template', () => {
                const template = '{{#if_template basic}}Basic template{{else}}Other template{{/if_template}}';
                const compiled = handlebars.compile(template);
        ...
        describe('Environment Helpers', () => {
            it('should check environment with if_env', () => {
                const template = '{{#if_env development}}Development{{else}}Other{{/if_env}}';
                const compiled = handlebars.compile(template);
        ...
        describe('Development Helpers', () => {
            it('should check development options', () => {
                const templates = {
                    ...
                    '{{#has_testing}}Testing{{else}}No Testing{{/has_testing}}': 'Testing'
                };
        ...
        describe('Plugin Helpers', () => {
            it('should check plugin existence', () => {
                const template = '{{#if_plugin "@farm/auth"}}Auth plugin{{else}}No auth plugin{{/if_plugin}}';
                const compiled = handlebars.compile(template);
    ...
    describe('Template Generation Integration', () => {
        it('should generate a complete project structure', async () => {
            ...
            const result = await scaffolder.generateProject(projectPath, testTemplateContext);
        ...
        it('should handle generation errors gracefully', async () => {
            ...
            const invalidContext = { ...testTemplateContext, template: 'non-existent-template' };
        ...
        it('should process complex templates with inheritance', async () => {
            const templatePath = path.join(tempDir, 'complex.hbs');
            const outputPath = path.join(tempDir, 'complex-output.txt');

            await fs.writeFile(templatePath, testTemplateContent);
    ...
    describe('Error Handling', () => {
        ...
        it('should handle invalid handlebars syntax', async () => {
            const templatePath = path.join(tempDir, 'invalid-syntax.hbs');
        ...
        it('should handle missing context properties', async () => {
            const templatePath = path.join(tempDir, 'missing-context.hbs');
            await fs.writeFile(templatePath, 'Hello {{missingProperty}}!');
            ...
            // Should still succeed but with empty value
            expect(result.success).toBe(true);
        });
    });
